...
ipcMain.handle('generate-pdf', async (event: IpcMainInvokeEvent, id: number) => {
  return await generatePdf(id)
})

ipcMain.handle('upload-csv', async (event: IpcMainInvokeEvent) => {
  const fileObject = await dialog.showOpenDialog({
    properties: ['openFile'],
  })
  if (mainWindow && !fileObject.canceled) {
    readAndPipeCsv(fileObject, mainWindow.webContents)
  }
})
